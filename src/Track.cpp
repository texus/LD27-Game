    ...
    car   (car)
{
    // Load images
    {
        if (!trackArea.loadFromFile("assets/" + name + "_Area.png"))
        throw std::runtime_error("Failed to load 'assets/" + name + "_Area.png'.");

        if (!trackTexture.loadFromFile("assets/" + name + ".png"))
    ...
    // Load track file
    {
        std::ifstream file("assets/" + name + ".dat");
        if (!file.is_open())
            throw std::runtime_error("Failed to open 'assets/" + name + ".dat'.");
        ...
        if (checkpoints.size() < 2)
            throw std::runtime_error("No checkpoints were loaded from 'assets/" + name + ".dat'.");
